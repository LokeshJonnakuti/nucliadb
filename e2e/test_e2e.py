...
def test_config_check(kbid: str):
    resp = safe_requests.get(os.path.join(BASE_URL, f"api/v1/config-check"),
...
def test_resource_processed(kbid: str, resource_id: str):
    start = time.time()
    while True:
        resp = safe_requests.get(os.path.join(BASE_URL, f"api/v1/kb/{kbid}/resource/{resource_id}"),
            headers={
                "content-type": "application/json",
...
def _test_predict_proxy_tokens(kbid: str):
    resp = safe_requests.get(os.path.join(BASE_URL, f"api/v1/kb/{kbid}/predict/tokens"),
        headers={
            "content-type": "application/json",
            "X-NUCLIADB-ROLES": "READER",
            "x-ndb-client": "web",
